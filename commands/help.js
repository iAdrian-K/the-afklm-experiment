"use strict"
const configUtils = require('../helper_services/config_helper')
const liveService = require('../helper_services/if_live_api_helper')
const messageCreator = require('../helper_services/message_creator');

module.exports = {
    name: "help",
    description: "Get the help message of the bot",
    async execute(message) {
      message.channel.send('If you reached here after typing a command, then it probably does not exist. Visit https://github.com/sanketpandia/the-afklm-experiment/blob/master/readme.md for the list of all commands.');
      return;
    }
}