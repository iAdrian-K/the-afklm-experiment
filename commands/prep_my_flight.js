...
module.exports = {
  ...
  async execute(message) {
    ...
    let userFlight = await ifHelper.getUserFlight(
    ...
    for (let i = 0; i < checklists.length; i++) {
      if (checklists[i].startsWith(userFlight))
        acceptedFiles.push("./assets_contents/checklists/" + checklists[i]);
    }
    if (acceptedFiles.length === 0) {
      message.channel.send(
        `Sorry I don't have the checklists for **${userFlight}**.`
      );
      return;
    }
